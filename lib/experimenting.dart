Future<void> main() async {
  ...
  buffer.writeln("static final _initialMap = <String, Nutrient>{");

  rawMap.forEach((key, value) {
    buffer.writeln("  '$key': Nutrient(");
    buffer.writeln("    unit: '${value["unit"]}',");

    if (value.containsKey("lowerLimit")) {
      buffer.writeln("    lowerLimit: ${value["lowerLimit"]},");
    }

    if (value.containsKey("upperLimit")) {
      buffer.writeln("    upperLimit: ${value["upperLimit"]},");
    }

    final translations = value["translations"] as Map<String, dynamic>;
    buffer.writeln("    translations: {");
    translations.forEach((lang, label) {
      buffer.writeln("      '$lang': '$label',");
    });
    buffer.writeln("    },");

    if (value.containsKey("tags")) {
      final tags = value["tags"] as List<dynamic>;
      final tagList = tags.map((tag) => "'$tag'").join(', ');
      buffer.writeln("    tags: [$tagList],");
    }
